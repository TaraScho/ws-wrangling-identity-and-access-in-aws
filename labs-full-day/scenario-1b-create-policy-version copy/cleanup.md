...
aws iam set-default-policy-version --policy-arn $POLICY_ARN --version-id v1 --profile taractf
aws iam delete-policy-version --policy-arn $POLICY_ARN --version-id v2 --profile taractf
```
...
aws iam delete-user-permissions-boundary \
  --user-name iamws-policy-developer-user --profile taractf

aws iam delete-role-permissions-boundary \
  --role-name iamws-policy-developer-role --profile taractf

aws iam delete-policy \
  --policy-arn arn:aws:iam::${ACCOUNT_ID}:policy/DeveloperBoundary --profile taractf
```
...
aws iam get-policy \
  ...
  --query 'Policy.DefaultVersionId' --output text --profile taractf
# expect: v1

aws iam get-user --user-name iamws-policy-developer-user \
  --query 'User.PermissionsBoundary' --output text --profile taractf
# expect: None
```
